<template>
  <a-layout class="h-100vh">
    ...
    >
      <div
        class="logo flex justify-start items-center h-[var(--logo-height)] px-2"
      >
      ...
      <div
        :class="collapsed ? 'transform-scale-70' : ''"
    ...
    <a-layout class="p-0">
      <div
        class="py-25px px-15px h-24px flex justify-between items-center bg-[var(--layout-bg-color)]"
      >
      ...
      <a-layout-content
        class="p-1 min-h-280px min-w-280px bg-[var(--bg-color)]"
      >
        <RouterView />
      </a-layout-content>
    </a-layout>
...
const useTheme = computed(() => themeStore.isDarkMode?'dark':'light');
...
const routerList = ref<any[]>([]); //路由列表

const useMemory = ref("0MB");

onMounted(() => {
  // 监听路由变化
  // console.log('onMounted', router.options.routes);
  themeStore.init();
  routerList.value.push(...router.options.routes);
  useMemory.value = showMem(); // 打印初始内存占用
